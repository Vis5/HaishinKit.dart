...
class Camera2Source(
    ...
    var eventListener: Camera2SourceEventListener? = null
    fun setCameraEventListener(l: Camera2SourceEventListener) {
        eventListener = l
        eventListener?.onException?.invoke(10)
    }
        ...
        private set(value) {
          try {
            ...
            field = value
              ...
              //onError(device, 71)
              eventListener?.onException?.invoke(11)
          } catch (e: Exception) {
              //onError(device, 71)
              eventListener?.onException?.invoke(11)
          }
        ...
        private set(value) {
          try {
            ...
            field?.let {
              ...
              //onError(device, 71)
              eventListener?.onException?.invoke(12)
          } catch (e: Exception) {
              //onError(device, 71)
              eventListener?.onException?.invoke(12)
          }
    ...
    fun switchCamera() {
        ...
        eventListener?.onException?.invoke(15)
        open(expect)
    }
...
class Camera2SourceEventListener {
    var onException : ((Int) -> Unit)? = null
}